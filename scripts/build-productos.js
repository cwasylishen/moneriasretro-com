...
const card = p => `<article class="card product${p.soldout ? " soldout out" : ""}"><a class="shot" href="/producto/${p.id}" tabindex="-1" aria-hidden="true">${p.soldout ? '<span class="flag">Agotado</span>' : ""}<img src="/assets/images/${p.img}" alt="${esc(p.alt)}" width="600" height="450" loading="lazy" /></a><div class="txt"><h3><a href="/producto/${p.id}">${esc(p.name)}</a></h3><p class="note">${esc(p.desc)}</p><div class="row"><span class="sticker price">${p.price}</span>${p.soldout ? `<a class="btn mini ghost" href="/#preventa">Ver preventa</a>` : `<a class="btn mini btn-wa" href="https://wa.me/50684804222?text=${p.wa}" rel="noopener">Comprar</a>`}</div></div></article>`;
